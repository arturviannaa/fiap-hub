...
export async function GET(req: Request) {
  ...
  const aba = new URL(req.url).searchParams.get('aba') === 'turma' ? 'turma' : 'minhas'
  const notas = await sql<Nota>(
    aba === 'turma'
      ? `SELECT n.id,n.titulo,n.corpo,n.publica,n.aula_slug,n.usuario_id,n.atualizado_em, u.nome AS autor, u.foto AS autor_foto
         FROM notas n JOIN usuarios u ON u.id=n.usuario_id WHERE n.publica ORDER BY n.atualizado_em DESC LIMIT 200`
      : `SELECT n.id,n.titulo,n.corpo,n.publica,n.aula_slug,n.usuario_id,n.atualizado_em, u.nome AS autor, u.foto AS autor_foto
         FROM notas n JOIN usuarios u ON u.id=n.usuario_id WHERE n.usuario_id=$1 ORDER BY n.atualizado_em DESC LIMIT 200`,
    aba === 'turma' ? [] : [u.id],
  )
...
export async function POST(req: Request) {
  ...
  await sql('INSERT INTO notas (usuario_id, aula_slug, titulo, corpo, publica) VALUES ($1,$2,$3,$4,$5)', [
    u.id, b.aula || null, String(b.titulo || '').trim().slice(0, 160), corpo, !!b.publica,
  ])
